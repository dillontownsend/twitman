...
export default async (req, res) => {
    ...
    if (session) {
        ...
        try {
            ...
            console.log(
                `${screen_name} retweeting ${tweets.statuses.length} tweet(s) by phrase, "${q}"`
            )
                ...
                setTimeout(async function () {
                    ...
                    try {
                        if (tweets.statuses[i].user.id_str != providerAccountId) {
                            ...
                            console.log(
                                `${screen_name} retweeted ${i + 1}/${tweets.statuses.length}`
                            )
                        }
                    } catch (err) {
                    ...
                    if (i < tweets.statuses.length) {
                        ...
                        console.log(`${screen_name} finished retweeting`)
                    }
